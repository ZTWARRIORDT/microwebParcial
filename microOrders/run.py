...
def register_service():
    try:
        ...
        service_id = f"orders-service-{os.getenv('HOSTNAME', 'localhost')}"
        c.agent.service.register(
            'orders-service',
            service_id=service_id,
            address='micro-orders',
            port=5000,
            check=consul.Check.http(
                f'http://micro-orders:5000/health',
                interval='10s'
            )
        )
...
def create_app():
    app = Flask(__name__, template_folder='orders/templates')
